...
function playTrack(name, card) {
  if (card === undefined || !card.classList.contains("playing")) {
    ...
    interval = setInterval(function () {
      if (audio !== undefined) {
        if (playerDiv.querySelector(".duration").innerText !== getTime(audio.duration)) playerDiv.querySelector(".duration").innerText = getTime(audio.duration)
...
function showMoreMusic(elem) {
  ...
  document
    ...
    .forEach((track) => track.classList.remove("hidden"))
}
...
playerDiv.querySelector(".slider").addEventListener("mousedown", (e) => {
  mousedown = true
  moveSlider(e)
})
document.addEventListener("mousemove", (e) => {
  if (mousedown) moveSlider(e)
})
